...
Python-модель для сравнения пяти обезличенных инвестиционных проектов четырьмя методами. Расчёты перенесены из дипломной Excel-модели в проверяемые функции, дополнены автоматическими тестами, рейтингом и визуализациями.
...
Проект Б используется как эталон, потому что в исходной дипломной модели для него сохранены промежуточные показатели всех четырёх методов. Автоматический тест сверяет NPV, PI, E(NPV), СКО, Poss(NPV >= 0), M(NPV), V(NPV) и V/M.

GitHub Actions запускает тесты при каждом push и pull request.

## Источник и ограничения

Модель основана на материалах выпускной квалификационной работы. Названия организаций и отраслевые признаки обезличены. В отдельных исходных Excel-листах по проекту Б встречались значения из разных версий расчёта; эталоном выбран полный обучающий лист.
